...
> 若覆寫 `chat_capacity`、`compare_capacity`、`finetune_base_capacity` 或 `guarded_capacity`，
> 執行 `Test-ModelAvailability.ps1` 時也必須傳入相同數值；預檢會把主 chat 與 guarded
> deployment 對同一模型消耗的容量**加總**（預設 `50 + 20 = 70 TPM`）。
...
| 主 chat model 部署（`parity` = `gpt-5.2`；`current` = `gpt-5.4`）| 模組 1 / 3 / 4 |
| 比較 model 部署（`parity` = `gpt-5-mini`；`current` = `gpt-5.4-mini`）| 模組 2 的並排比較 |
| `gpt-4.1-mini` 部署 | 模組 5 微調的「微調前」基準 |
| `<chat model>-guarded` 部署 + 自訂 guardrail | 模組 6（`parity` = `gpt-5.2-guarded`；`current` = `gpt-5.4-guarded`；**刻意獨立**，嚴格門檻不會影響其他 demo）|
...
| `Start-FineTune.ps1` | **開課前 1–2 天，手動** | `gpt-4.1-mini` 監督式微調（60 分鐘以上）；等待 Developer-tier `provisioningState=Succeeded` 才回報可推論 |
| `Remove-FineTuneDeployment.ps1` | `destroy` 期間自動 | 刪除不在 Terraform state 的手動微調部署，避免 `CannotDeleteAccountWithDeployments` |
...
$model   = terraform output -raw model_deployment
$guarded = terraform output -raw guarded_deployment
$h       = @{ Authorization = "Bearer $tok" }

# Main chat deployment: works with either model_profile.
$r = Invoke-RestMethod -Method POST -Uri "$base/responses" -Headers $h `
     -ContentType 'application/json' `
     -Body (@{ model = $model; input = 'Reply with exactly: OK' } | ConvertTo-Json)
($r.output | Where-Object type -eq 'message').content.text

# $guarded is the independently guarded Module 6 deployment.
```
...
| 檢查 | 預期 |
|---|---|
| `terraform output -raw model_deployment` 回應 | 正常回覆 |
...
| `terraform output -raw guarded_deployment` | Foundry portal 中該部署顯示套用 `ai3016-strict-guardrail` |
...
└─ scripts/
   ...
   └─ Remove-FineTuneDeployment.ps1 手動微調部署的 destroy cleanup
```
